<template>
	<div>
        ...
        <el-table :data="rds.rdsinfo.data" 
        ...
        >
            ...
            <el-table-column prop="address_port" label="地址"  align="center" >
                <template #default="scope">
...
<script setup lang="ts" name="rds_list">
import { ref,onMounted } from 'vue';
import { userds_liststore } from '@/store/ad_tracking/rds_list';
import { Refresh} from '@element-plus/icons-vue';
import * as XLSX from 'xlsx';
import { ElTable } from 'element-plus'
import { ElMessage, ElMessageBox, ElButton, ElLoading } from 'element-plus'
import {all_rds as expall_rds} from '@/http/ad_tracking/nginx'
import type { FormInstance, FormRules } from 'element-plus'

const rds = userds_liststore();

// 获取表格数据,点击菜单后才加载
onMounted(() => {
    rds.getall_rds(query.value)
})
...
const handleSearch = (formEl: FormInstance | undefined) => {
    ...
    await rds.getall_rds(query.value)
    ElMessage({
            message: rds.rdsinfo.msg,
            type: 'success',
        })
    } else {
      return false
...
// getSelectionRows  Element Plus table表格组件方法，获取当前选中的数据
  ...
  // 创建工作表
  const headers =  ['序号','实例ID', '名称', '状态', '磁盘', '内存','版本', '域名', '地址', '端口', '类型', '创建时间'];
  const worksheetData = [  
  headers,  
  ...
  XLSX.utils.book_append_sheet(new_workbook, WorkSheet, 'RDS服务列表');  

  // 将新的工作簿保存为 Excel 文件  
  XLSX.writeFile(new_workbook, `RDS服务列表.xlsx`);  
  loading.close() // 关闭遮罩层
};
...
const Exportall = async () => {
    ...
    let list: any = []; // 用于存储所有操作日志数据
     // 获取第一页数据
    // 计算总页数
    const totalPages = Math.ceil(rds.rdsinfo.count/10) 
    console.log(totalPages)
    for (let pageNum = 1; pageNum <= totalPages; pageNum++) {
        query.value.pagenum = pageNum
        const res = await expall_rds(query.value)
     ...
     ['序号','实例ID', '名称', '状态', '磁盘', '内存','版本', '域名', '地址', '端口', '类型', '创建时间'],
        ...list
    ]);
